...
router.post('/', async (req, res) => {
  try {
    ...
    const vrifycode = otpGenerator.generate(5, { lowerCaseAlphabets: false, upperCaseAlphabets: false, specialChars: false });
    console.log(vrifycode);

    const password = await bcrypt.hash(req.body.password, 10);
    const user = new Temprary({ ...req.body, password, vrify: false });
    ...
    if (newUser) {
      ...
      res.status(200).send({ status_code: 200, message: "Verification email sent successfully" });
    } else {
      res.status(403).send({ message: "Timeout or other issue during user creation" });
    }
